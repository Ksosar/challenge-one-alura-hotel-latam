...
public class Busqueda extends JFrame {
	...
	public Busqueda() {
		...
		String[] listaColumnasReservas = { "ID", "fechaentrada", "fechasalida", "PAGO", "PRECIO", "HUESPED", "HUESPED_ID" };
		...
		btnBuscar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (txtCampoBusqueda.getSelectedItem().toString() == "IDRESERVA") {
	...
	private void modificar() {
		if (tbHuespedes.isVisible()) {
			if (tieneFilaElegida(tbHuespedes) || tbHuespedes.getSelectedRow() == 0) {
				...
				Integer idreserva = Integer.valueOf(tbHuespedes.getValueAt(fila, 6).toString());
		...
		if (tbReservas.isVisible()) {
			...
			if (tieneFilaElegida(tbReservas) || tbReservas.getSelectedRow() == 0) {
				...
				try {
					fechaentrada = LocalDate.parse(tbReservas.getValueAt(fila, 1).toString());
					fechasalida = LocalDate.parse(tbReservas.getValueAt(fila, 2).toString());
				} catch (DateTimeParseException e) {
					JOptionPane.showMessageDialog(this, "La fecha modificada no es una fecha válida");
					return;
				}

				if (fechaentrada.until(fechasalida, ChronoUnit.DAYS) <= 0) {
					JOptionPane.showMessageDialog(this, "El Chek-in no puede ser posterior al Chek-out");
					return;
				}
				Integer valor = Integer.parseInt(tbReservas.getValueAt(fila, 4).toString());
				String formapago = tbReservas.getValueAt(fila, 3).toString();
				Reserva reserva = new Reserva(id, fechaentrada, fechasalida, formapago, valor);
	...
	private void cargarTablaHuesped() {
		...
		huespedes.stream().sorted(Comparator.comparing(Huesped::getApellido))
				.forEach(huesped -> modeloHuesped.addRow(new Object[] { huesped.getId(), huesped.getApellido(),
						huesped.getNombre(), huesped.getfechanacimiento().toString(), huesped.getNacionalidad(),
...
//				huesped.getTelefono(), huesped.getId_reserva() }));
	...
	private void cargarTablaReserva() {
		...
		reservas.stream().sorted(Comparator.comparing(Reserva::getfechaentrada)).forEach(
				reserva -> modeloReserva.addRow(new Object[] { reserva.getId(), reserva.getfechaentrada().toString(),
						reserva.getfechasalida().toString(), reserva.getformapago(), reserva.getvalor(),
...
//				reserva.getfechaentrada().toString(), reserva.getfechasalida().toString(), reserva.getPago(), reserva.getPrecio(),
//				reserva.getHuesped().getApellido() + " " + reserva.getHuesped().getNombre(),
//				reserva.getHuesped().getId() }));
	}
	...
	private void cargarTablasBusqueda(List<Reserva> reservas) {
		...
		reservas.forEach(reserva -> modeloReserva.addRow(new Object[] { reserva.getId(),
				reserva.getfechaentrada().toString(), reserva.getfechasalida().toString(), reserva.getformapago(), reserva.getvalor(),
				reserva.getHuesped().getApellido() + " " + reserva.getHuesped().getNombre(),
				reserva.getHuesped().getId() }));

		reservas.forEach(reserva -> modeloHuesped.addRow(new Object[] { reserva.getHuesped().getId(),
				reserva.getHuesped().getApellido(), reserva.getHuesped().getNombre(),
				reserva.getHuesped().getfechanacimiento().toString(), reserva.getHuesped().getNacionalidad(),
				reserva.getHuesped().getTelefono(), reserva.getHuesped().getIdreserva() }));
